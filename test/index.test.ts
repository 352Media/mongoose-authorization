...
import PermissionDeniedError = require('../src/PermissionDeniedError');
...
const {permissions} = (User.schema as AuthorizationSchema);
...
module.exports = {
    'setUp': async (callback) => {
        try {
            ...
            const user1 = await new User(userSeed1).save({authLevel: false} as any);
            const user2 = await new User(userSeed2).save({authLevel: false} as any);
            user1.best_friend = user2;
            await user1.save({authLevel: false} as any);
    ...
    'Disable': {
        find: async (test) => {
            const users = await (User.find() as any).setAuthLevel(false).exec();
            test.ok(users);
        ...
        create: async (test) => {
            try {
                ...
                await testCreation.save({authLevel: false} as any);
            } catch (err) {
                test.ifError(err);
            }
            test.done();
    ...
    'Finding Documents': {
        ...
        'Filtered Fields': async (test) => {
            try {
                const users = await (User.find() as any).setAuthLevel(['admin', 'owner']).exec();
                test.ok(users);
                test.equal(users.length, 2);
        ...
        'findOne': async (test) => {
            try {
                const user = await (User.findOne({email: 'foo@example.com'}) as any)
                    .setAuthLevel('admin')
                    .exec();
                test.ok(user);
        ...
        'with permissions option: named': async (test) => {
            ...
            test.ok(user);
            test.deepEqual(
                (user as any).perms.read.sort(),
                ['_id', 'email', 'first_name', 'last_name', 'avatar', 'status', 'best_friend'].sort(),
            );
            test.deepEqual(
                (user as any).perms.write.sort(),
                ['status', 'primary_location'].sort(),
            );
            test.strictEqual(
                (user as any).perms.remove,
                false,
            );
            test.done();
        },
    ...
    'Updating Documents': {
        ...
        'findOneAndUpdate with `new` option': async (test) => {
            const user = await User.findOneAndUpdate(
                {_id: userDocs[0]._id},
                {status: 'disabled'},
                {new: true, authLevel: 'admin'}  as any,
            );
